...
from rag import RagPromptBuilder, RagRetriever, build_rag_metrics, print_metrics
...
def find_largest_gap_k(
    ...
    *,
    ...
    start = int((scores.size - 1) * ignore_head) if isinstance(ignore_head, float) else int(ignore_head)
    tail = int((scores.size - 1) * ignore_tail) if isinstance(ignore_tail, float) else int(ignore_tail)
    start = max(0, min(start, scores.size - 2))
    tail = max(0, min(tail, scores.size - start - 2))

    gaps = np.diff(scores)
    candidate_gaps = gaps[start:] if tail == 0 else gaps[start:-tail]
    if candidate_gaps.size == 0:
        base_k = min_k
    else:
        threshold = int(np.argmin(candidate_gaps) + start)
        base_k = threshold + 1

    if isinstance(retrieve_more, float):
        k = int(base_k * retrieve_more)
    else:
...
class AdaptiveRagChatTurn:
    ...
    def prepare(self, query):
        if self.search_k <= 0:
            raise ValueError("rag_search_k must be positive")
...
def main(argv=None, default_awq_backend="auto"):
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        help="Number of candidates to search before adaptive-k filtering.",
    )
    parser.add_argument(
        "--adaptive_strategy",
        choices=["largest_gap"],
        default="largest_gap",
        help="Adaptive-k strategy. This script currently implements largest_gap.",
    )
    parser.add_argument(
        "--adaptive_ignore_head",
        type=float,
        default=0.0,
        help="Fraction of highest-ranked score gaps to skip before finding the largest gap.",
    )
    parser.add_argument(
        "--adaptive_ignore_tail",
        type=float,
        default=0.1,
        help="Fraction of lowest-ranked score gaps to skip before finding the largest gap.",
    )
    parser.add_argument(
        "--adaptive_retrieve_more",
        type=int,
        default=5,
        help="Extra chunks to retrieve after the largest-gap threshold.",
    )
    parser.add_argument(
        "--adaptive_min_k",
        type=int,
        default=1,
        help="Minimum number of retrieved chunks after adaptive-k filtering.",
    )
    parser.add_argument(
        "--adaptive_max_k",
        type=int,
        default=None,
        help="Maximum number of retrieved chunks after adaptive-k filtering. Defaults to rag_search_k.",
    )
    parser.add_argument(
        "--rag_prompt_template",
        default=None,
