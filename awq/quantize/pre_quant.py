...
def _get_attn_head_dim(layer):
    ...
    if q_norm_weight is not None:
        return q_norm_weight.numel()

    q_proj = getattr(self_attn, "q_proj", None)
    config = getattr(self_attn, "config", None)
    num_heads = getattr(self_attn, "num_heads", None) or getattr(
        config, "num_attention_heads", None
    )
    if q_proj is not None and num_heads:
...
def _sanitize_layer_kwargs(layer, layer_kwargs):
    kwargs = dict(layer_kwargs)
    position_embeddings = kwargs.get("position_embeddings")
    head_dim = _get_attn_head_dim(layer)
    if position_embeddings is None or head_dim is None:
        return kwargs
    ...
    if cos.shape[-1] < head_dim:
        raise RuntimeError(
            "position_embeddings last dimension is smaller than attention head_dim: "
            f"{cos.shape[-1]} < {head_dim}"
        )

    kwargs["position_embeddings"] = (
        cos[..., :head_dim].contiguous(),
        sin[..., :head_dim].contiguous(),
    )
    return kwargs
...
def run_awq(
    ...
    for i in tqdm.tqdm(range(len(layers)), desc="Running AWQ..."):
        ...
        cur_layer_kwargs = _sanitize_layer_kwargs(layer, layer_kwargs)
        # get output as next layer's input
        layer_out = layer(inps, **cur_layer_kwargs)
