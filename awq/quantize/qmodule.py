...
def _get_awq_inference_engine():
    try:
        ...
        raise ModuleNotFoundError(
            "awq_inference_engine is required only for running packed W4A16 "
            "AWQ inference. Build awq/kernels before loading and generating "
            "with a real quantized checkpoint."
        ) from exc
...
class WQLinear(nn.Module):
    ...
    @torch.no_grad()
    def forward(self, x):
        awq_inference_engine = _get_awq_inference_engine()
